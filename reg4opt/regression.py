...
def operator_regression(x, y, c, autotune=False, solver=None, **solver_params):
    ...
    problem.solve(solver, **solver_params)
...
#%% OPERATOR REGRESSION (PRS)

def _P_inverse_or(p_0, p_1, n):
...
def qcqp_or(p_0, q_0, P, r,
    ...
    for k in range(int(num_iter)):
        ...
        # check stopping condition
        if (l-l_old)**2 <= tol: break

    return - v_1, l

def operator_regression_prs(x, y, zeta, tol=1e-2, num_iter=1e3, rho=1, newton_params=None):
    """
    """
    ...
    for k in range(int(num_iter)):
        ...
        for e in E:
            ...
            xi[e], _ = qcqp_or(p_0, q_0[e] - z[e]/rho, P, r[e], **newton_params)
...
# ------------ operator regression with contraction constant autotuning
def _P_inverse_or_autotuning(p_0_1, p_0_2, p_1, n):
...
def qcqp_or_autotuning(p_0_1, p_0_2, q_0, P, q,
                l=0, tol=1e-4, num_iter=25, b=0.8, c=0.5, max_iter=10):
    ...
    q_l = q_0 + l*q
    P_inv = _P_inverse_or_autotuning(p_0_1, p_0_2, l, n)
    v_1 = P_inv.dot(q_l)
...
def operator_regression_prs_autotuning(x, y, weight, tol=1e-2, num_iter=1e3, rho=1, eps=1e-10, newton_params=None):
    """
    """
    ...
    for k in range(int(num_iter)):
        ...
        for e in E:
            ...
            xi[e], _ = qcqp_or_autotuning(p_0_1, p_0_2, q_0[e] - z[e]/rho, P, q[e], **newton_params)
